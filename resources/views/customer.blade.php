        ...
        <form action="{{ route('customer.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <input type="text" name="nama" placeholder="Example: Tefa" value="{{ old('nama') }}"
                ...
                <div>
                    ...
                    <input type="text" name="alamat" placeholder="Example: Jl. Merdeka No. 10, Jakarta" value="{{ old('alamat') }}"
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-2">
                ...
                <h3 class="text-lg font-bold text-slate-800 tracking-tight">Search Customer</h3>
            </div>
            <button class="flex items-center gap-2 bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg text-sm font-semibold transition"
                    onclick="window.location.href='{{ route('customer.export.pdf') }}{{ $search ? '?search=' . urlencode($search) : '' }}'">
        ...
        <form action="{{ route('customer.index') }}" method="GET" class="flex gap-3">
            <div class="flex-1">
                <input type="text" name="search" value="{{ $search ?? '' }}" placeholder="Search by customer name, address or phone number..."
            ...
            @if($search)
                <a href="{{ route('customer.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-2 rounded-lg text-sm font-semibold transition">
                    Reset
                </a>
            @endif
        </form>
        ...
        @if($search)
            <div class="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <p class="text-sm text-blue-800">
                    ...
                    <span class="text-blue-600">({{ $customers->total() }} results found)</span>
                </p>
            </div>
        @endif
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="flex items-center justify-between p-6 border-b border-gray-100">
            <div class="flex items-center gap-2">
                ...
                <h3 class="text-lg font-bold text-slate-800 tracking-tight">
                    Customer List (Total: {{ $customers->total() }})
                </h3>
            </div>
        </div>
        ...
        <div class="flex justify-center gap-2">
            ...
            {{-- DELETE --}}
            <button
                onclick="openDeleteModal({{ $customer->id }}, '{{ $customer->nama }}')"
        ...
        <div class="px-6 py-4 border-t border-gray-100 flex items-center justify-between bg-gray-50">
            <div class="text-sm text-gray-600">
                Showing <span class="font-bold">{{ $customers->count() }}</span> from <span class="font-bold">{{ $customers->total() }}</span> customer
            </div>
            <div class="flex items-center gap-2">
...
<div id="editModal" class="fixed inset-0 bg-black/40 hidden items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-md">
        ...
        <form id="editForm" method="POST">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-xs font-semibold text-gray-400 mb-1 uppercase tracking-wider">Customer Name</label>
...
<div id="deleteModal" class="fixed inset-0 bg-black/40 hidden items-center justify-center z-50 p-4">
    <div class="bg-white rounded-xl shadow-lg p-6 w-full max-w-sm">
        ...
        <h3 class="text-lg font-bold text-center text-slate-800 mb-2">Delete Customer?</h3>
        <p class="text-center text-gray-600 text-sm mb-6">
            Are you sure you want to delete customer <br><span id="deleteName" class="font-semibold text-slate-800"></span>?<br> This action cannot be undone.
        </p>
